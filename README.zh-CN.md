...
![VibeX IADE：内置 Agent 与 ACP Registry Agent 经 ACP 接入同一 Host，并由 WebUI、Desktop、CLI、Mobile APP 使用](./docs/readme/iade-architecture.svg)
...
| **CLI** | `npx @xircth/vibex` 按平台拉取 Host 家族包、校验校验和并启动 Server。 |
...
`npx @xircth/vibex` 按平台拉取 `VibeX-${VERSION}-{linux-x86_64,linux-aarch64,darwin-aarch64,windows-x86_64,windows-aarch64}-server.tar.gz`，核对 sidecar 的 `.sha256` 与包内 `SHA256SUMS`，再启动 `vibex-server`，并把 `VIBEX_STATIC_ROOT` 指到包内 `web/`。同一份归档也可以用 `install.sh` / `install.ps1` 安装。

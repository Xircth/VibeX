...
3. 连接。插件会安装并启动远端 Host（`vibex serve`），经 SSH 隧道配对，写入「设置 → 远程连接」，并打开新窗口接入该 Host。远端已有自行配置、正在运行的 Server 时直接复用，不覆盖。当前窗口保持本机工作不变。
...
远端安装使用官方 Host Family：本机下载后经 SSH 上传。远端没有 Node.js 时，连接过程会把 Node 和 npm 装进用户环境。Linux 需要 glibc 2.34+（Ubuntu 22.04 / RHEL 9）。

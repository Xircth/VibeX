...
// something on the page and hand it to a conversation. This is not a
// document-start script: a page nobody picks on never pays for
...
;(function codegBrowserPicker() {
  ...
  function post(message) {
    try {
      var queue = globalThis.__vibexPickQueue
      if (!Array.isArray(queue)) {
        queue = []
        globalThis.__vibexPickQueue = queue
      }
      queue.push(message)
    } catch {
      /* the queue is a fallback when ipc is missing */
    }
    try {
      send(message)
    } catch {
  ...
  function ensureOverlay() {
    ...
    try {
      ...
      style.textContent =
        ...
        "border:1px solid rgba(59,130,246,.95);background:rgba(59,130,246,.16);border-radius:2px}" +
        ...
        "background:rgb(37,99,235);color:#fff;font:11px/1.6 ui-monospace,SFMono-Regular,Menlo,monospace}"
  ...
  // escape hatch: the flag lives in this world, which page

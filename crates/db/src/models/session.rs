...
impl Session {
    ...
    ///
    /// Sessions that already contain coding-agent executions are always prioritized
    /// above empty/helper-only sessions. Within each group, sessions are sorted by
    /// their latest coding-agent execution timestamp, or by created_at when no
    /// coding-agent execution exists.
    ...
    /// Find the most recently used coding session for a workspace.
    ///
    /// Sessions with coding-agent history are preferred over empty/helper-only
    /// sessions. If no coding session exists, falls back to the newest created
    /// session.

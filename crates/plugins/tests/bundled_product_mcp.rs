...
use plugins::{
    ConflictDecision, InMemoryPluginRegistry, MULTI_AGENT_PLUGIN_ID, PluginControlPlane,
    PluginPackage, PluginSourceKind, SESSION_ENHANCE_PLUGIN_ID, SESSION_FEAT_ALL, SESSION_FEAT_ASK,
};
...
fn bundled_multi_agent_plugin_is_the_delegation_gate() {
    ...
    assert_eq!(package.id.as_str(), MULTI_AGENT_PLUGIN_ID);
...
fn bundled_session_enhance_plugin_carries_four_tool_switches() {
    ...
    assert_eq!(package.id.as_str(), SESSION_ENHANCE_PLUGIN_ID);
...
async fn enabling_each_plugin_opens_only_its_mcp_gate() {
    ...
    control
        .set_enabled(MULTI_AGENT_PLUGIN_ID, true)
        .await
        .unwrap();
    assert!(gate.allow_delegation_mcp());
    assert!(!gate.allow_session_mcp());
    ...
    control
        .set_enabled(SESSION_ENHANCE_PLUGIN_ID, true)
        .await
        .unwrap();
    assert!(gate.allow_session_mcp());
    ...
    control
        .set_enabled(MULTI_AGENT_PLUGIN_ID, false)
        .await
        .unwrap();
    assert!(!gate.allow_delegation_mcp());
    assert!(gate.allow_session_mcp());

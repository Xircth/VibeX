//! Install Git into the Host user environment from the dugite-native releases.
...
pub(super) fn managed_install_root() -> PathBuf {
    utils::assets::asset_dir().join("tools").join("git")
}

pub(super) fn find_managed_executable() -> Option<PathBuf> {
    find_git_executable(&managed_install_root())
}

pub(super) async fn install() -> Result<PathBuf, String> {
...
pub(super) async fn configure_identity(
    git_path: &Path,
    user_name: &str,
    user_email: &str,
) -> Result<(), String> {
...
mod tests {
    ...
    #[test]
    fn finds_git_inside_nested_distribution_root() {
        let root = tempfile::tempdir().expect("temp dir");
        let nested = root.path().join("dugite");
        let executable_name = if cfg!(windows) { "git.exe" } else { "git" };
        let bin = nested.join("bin");
        std::fs::create_dir_all(&bin).expect("bin dir");
        let executable = bin.join(executable_name);
        std::fs::write(&executable, b"git").expect("git stub");
        assert_eq!(
            find_git_executable(root.path()).as_deref(),
            Some(executable.as_path())
        );
    }

    #[test]
    fn unpacks_tar_gz_and_locates_git() {
        let dest = tempfile::tempdir().expect("dest");
        let executable_name = if cfg!(windows) { "git.exe" } else { "git" };
        let archive = tar_gz_with_file(&format!("bin/{executable_name}"), b"git-stub");
        unpack_tar_gz(&archive, dest.path()).expect("unpack");
        assert_eq!(
            find_git_executable(dest.path()),
            Some(dest.path().join("bin").join(executable_name))
        );
    }

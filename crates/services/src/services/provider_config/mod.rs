...
async fn read_store_json<T>(path: PathBuf) -> Result<T, ProviderConfigError>
where
...
async fn write_store_json<T>(path: PathBuf, value: &T) -> Result<(), ProviderConfigError>
where
    T: Serialize,
{
    if let Some(parent) = path.parent() {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            ProviderConfigError::Internal(format!("Failed to create {}: {e}", parent.display()))
        })?;
    }
...
async fn save_secrets(secrets: &ProviderSecrets) -> Result<(), ProviderConfigError> {
...
fn atomic_write_bytes(path: &Path, bytes: &[u8]) -> Result<(), ProviderConfigError> {
    if let Some(parent) = path.parent() {
...
///
/// Fail-loud (代码报告 1.3 / H-3): previously this returned `()` and swallowed every
/// failure, after which the caller overwrote the user's real CLI config anyway —
/// silently destroying it when the backup didn't happen. Now any backup failure is
/// propagated so the caller aborts the write. A missing source file is not an error:
/// there is nothing to back up before writing a brand-new config.
fn backup_existing(path: &Path, agent: &str) -> Result<(), ProviderConfigError> {
    ...
    let home = dirs::home_dir().ok_or_else(|| {
        ProviderConfigError::Internal(
            "cannot resolve home directory to back up the existing provider config".to_string(),
        )
    })?;
    let dir = home.join(".vibex").join("provider-backups").join(agent);
    std::fs::create_dir_all(&dir).map_err(|e| {
        ProviderConfigError::Internal(format!(
            "failed to create provider-config backup dir {}: {e}",
            dir.display()
        ))
    })?;
    let base = path
...
fn read_json_file(path: &Path) -> Result<Value, ProviderConfigError> {
    if !path.exists() {
        return Ok(json!({}));
    }
...
fn read_yaml_file(path: &Path) -> Result<serde_yaml::Value, ProviderConfigError> {
    if !path.exists() {
...
fn json_content(value: &Value) -> Result<String, ProviderConfigError> {
    let serialized = serde_json::to_string_pretty(value)
        .map_err(|e| ProviderConfigError::Internal(format!("Failed to serialize JSON: {e}")))?;
    Ok(format!("{serialized}\n"))
}

fn toml_content(value: &toml::Value) -> Result<String, ProviderConfigError> {
    toml::to_string_pretty(value)
        .map_err(|e| ProviderConfigError::Internal(format!("Failed to serialize TOML: {e}")))
}

fn yaml_content(value: &serde_yaml::Value) -> Result<String, ProviderConfigError> {
    serde_yaml::to_string(value)
        .map_err(|e| ProviderConfigError::Internal(format!("Failed to serialize YAML: {e}")))
}
...
fn render_provider_config(
    ...
    api_key: Option<&str>,
) -> Result<Vec<RenderedFile>, ProviderConfigError> {
    match agent {
...
fn apply_codex_provider_to_config(
    ...
    if has_api_key {
        ...
        let providers = providers.as_table_mut().ok_or_else(|| {
            ProviderConfigError::BadRequest("codex model_providers 必须是 table".to_string())
        })?;
...
fn render_hermes(
    ...
    let mut root = read_yaml_file(&path)?;
    {
        let map = root.as_mapping_mut().ok_or_else(|| {
            ProviderConfigError::BadRequest("hermes config.yaml 根节点必须是 mapping".to_string())
        })?;
        ...
        let seq = map
            ...
            .ok_or_else(|| {
                ProviderConfigError::BadRequest("hermes custom_providers 必须是序列".to_string())
            })?;
...
pub async fn update_agent_provider(
    ...
    let is_current;
    {
        let agent_providers = store
            .agents
            .get_mut(&agent)
            .ok_or_else(|| ProviderConfigError::NotFound(format!("供应商不存在：{provider_id}")))?;
...
pub async fn fetch_agent_provider_models(
    agent_type: String,
    provider_id: String,
) -> Result<ProviderModelsResult, ProviderConfigError> {
    let agent = normalize_agent(&agent_type)?;
    let store = load_store().await?;
    let secrets = load_secrets().await?;
    ...
    let response = request
        .send()
        .await
        .map_err(|e| ProviderConfigError::Internal(format!("获取模型列表失败：{e}")))?;
    let status = response.status();
    if !status.is_success() {
        let detail = response.text().await.unwrap_or_default();
        let message = format!("供应商返回 {status}：{detail}");
        // 5xx is the provider's fault, not the caller's — classify as Internal so the
        // shell doesn't mislabel an upstream outage as a client BadRequest (代码报告 4.2).
        return Err(if status.is_server_error() {
            ProviderConfigError::Internal(message)
        } else {
            ProviderConfigError::BadRequest(message)
        });
    }
    ...
    let parsed: OpenAiModelsResponse = response
        .json()
        .await
        .map_err(|e| ProviderConfigError::Internal(format!("解析模型列表失败：{e}")))?;

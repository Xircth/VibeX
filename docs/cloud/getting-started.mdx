...
VibeX Cloud lets you collaborate with your team in real-time. Your data syncs to the cloud, so everyone sees the same projects, issues, and updates.

<Info>
**Already using VibeX locally?** Cloud uses the same interface you're familiar with. The difference is your data syncs to the cloud and you can invite team members.
</Info>

## Step 1: Launch VibeX

If you haven't already, start VibeX:
...
<Step title="Authorise the application">
  You'll be redirected to GitHub or Google. Sign in if needed, then click **Authorize** or **Allow** to grant VibeX access.

  <Info>
  VibeX only requests access to your basic profile (name, email, avatar). It cannot access your private repositories or post on your behalf.
  </Info>
</Step>

<Step title="Return to VibeX">
...
Both appear in the same VibeX interface. Local projects show in one section, Cloud projects in another.

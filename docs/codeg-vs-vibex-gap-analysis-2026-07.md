...
| **P2-5** 系统托盘 + 深链 | ✅ 完成 | Tauri v2 内置 tray-icon（显示/隐藏/退出 + 左键 + 活动徽标）+ vibex:// 深链（single-instance→deep-link，复用 desktop-toast-activated 通道，三平台 bundle 注册）|
| **P3-2** 等宽字体选择 | ✅ 完成 | --font-mono token 收敛（tailwind + 全部硬编码族）+ 外观页字体选择器 + xterm 实时联动 |
| **P3-1** i18n 全量前端 | ✅ 完成 | 8 命名空间（common/settings/statusbar/app/tasks/dialogs/conversation/panels）覆盖 ~180 文件、~1500 键；9 批 agent 并行转换 + 中心合并 + 键集对等；**1873 处 t() 键引用零漂移**（键完整性校验）；测试基建 i18n 初始化。剩余仅：后端用户可见串 + localStorage→Config.language 同步 |
...
**合计已交付 29 项**（整个 P0 层 + P1 全部 + 多数 P2 + 多数 P3），全部经 `cargo clippy --workspace --features qa-mode -D warnings`/后端全量测试(0 失败)/前端 785 测试/`generate-types:check`/`prepare-db:check` 验证全绿；三轮对抗性审查（automation 3 + fork/composer 4 + i18n/P3-2/P3-5 4）确认缺陷均已修复。
...
- **i18n 收尾**：后端用户可见串（IM 模板/恢复原因等）i18n 化 + localStorage→Config.language 同步（使后端串也随前端切换）为后续一批；前端已全量。

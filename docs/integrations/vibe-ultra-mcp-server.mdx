---
title: "Vibe Ultra MCP Server"
description: "Configure the Vibe Ultra MCP server"
---

Vibe Ultra exposes a local MCP (Model Context Protocol) server, allowing you to manage projects and tasks from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Vibe Ultra itself.

<Note>
This page covers connecting **external MCP clients** to Vibe Ultra's MCP server. For configuring MCP servers **within** Vibe Ultra for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Vibe Ultra's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Vibe Ultra MCP server to any [supported coding agent](/supported-coding-agents) **within** Vibe Ultra.

1. In Vibe Ultra Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Vibe Ultra card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.jpeg" alt="MCP Servers configuration page showing how to add Vibe Ultra MCP server" />
</Frame>
...
The Vibe Ultra MCP server provides the following tools for managing projects, tasks, and task execution:
...
3. **Automatic Task Generation**: Your MCP client will use the Vibe Ultra MCP server to automatically create structured tasks in your project
...
Your MCP client will use the `create_task` tool to break this down into individual tasks and add them to your Vibe Ultra project automatically.
...
### Internal Coding Agents (Within Vibe Ultra)

A powerful workflow involves using coding agents within Vibe Ultra that are also connected to the Vibe Ultra MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual tasks directly in the Vibe Ultra interface
...
Raycast is a popular MCP client that can connect to Vibe Ultra's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding Vibe Ultra server" />
</Frame>

Configure the Vibe Ultra MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the Vibe Ultra MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>

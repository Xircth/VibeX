---
title: "VibeX MCP Server"
description: "Configure the VibeX MCP server"
---

VibeX exposes a local MCP (Model Context Protocol) server, allowing you to manage projects and tasks from external MCP clients like Claude Desktop, Raycast, or even coding agents running within VibeX itself.

<Note>
This page covers connecting **external MCP clients** to VibeX's MCP server. For configuring MCP servers **within** VibeX for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
VibeX's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the VibeX MCP server to any [supported coding agent](/supported-coding-agents) **within** VibeX.

1. In VibeX Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the VibeX card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.jpeg" alt="MCP Servers configuration page showing how to add VibeX MCP server" />
</Frame>
...
The VibeX MCP server provides the following tools for managing projects, tasks, and task execution:
...
3. **Automatic Task Generation**: Your MCP client will use the VibeX MCP server to automatically create structured tasks in your project
...
Your MCP client will use the `create_task` tool to break this down into individual tasks and add them to your VibeX project automatically.
...
### Internal Coding Agents (Within VibeX)

A powerful workflow involves using coding agents within VibeX that are also connected to the VibeX MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual tasks directly in the VibeX interface
...
Raycast is a popular MCP client that can connect to VibeX's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding VibeX server" />
</Frame>

Configure the VibeX MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the VibeX MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>

...
- 2026-06-13 / Phase 2 T2.15：完成 overlayscrollbars 裁剪记录。Phase 2 保留原生滚动条，不新增 `overlayscrollbars`/`overlayscrollbars-react`，避免在虚拟 rows、stick-to-bottom、`scrollToIndex`、键盘滚动与文本选择尚需浏览器回归时引入额外 scroll wrapper。后续在具备浏览器滚动/选区回归后再于视觉/外观阶段评估。决策见 `02-conversation-rendering/overlayscrollbars-decision.md`。验证：`rg "overlayscrollbars" frontend package.json pnpm-lock.yaml` 无命中，生产运行时代码和依赖文件未接入。
- 2026-06-13 / Phase 2 T2.14：完成 Composer 命令源与队列 UI 收口。新增 `lib/conversation-rendering/commandSources.ts`，统一 `/` 内置命令、Phase 1 `AvailableCommands` 与本地 skills 的 source registry，并保留 `@` 工作区文件 typeahead 路径；`SessionComposerInput` 接入会话 scope 的 runtime commands 与本地 skills，避免破坏 draft scratch。`MessageQueueIndicator` 升级为可展开的队列消息预览，支持编辑、删除、附件/图片路径展示；当前后端仅支持单条 queued prompt，因此重排控件以禁用态展示并记录为后续多队列模型事项。验证：`cd frontend && pnpm exec vitest run src/lib/conversation-rendering/commandSources.test.ts src/components/tasks/follow-up/sessionComposerTypeahead.test.ts src/components/tasks/follow-up/sessionComposerQueue.test.ts src/components/tasks/follow-up/UseSessionComposerQueue.test.tsx src/components/tasks/follow-up/MessageQueueIndicator.test.tsx src/components/tasks/follow-up/SessionComposerInput.test.tsx` 45 条通过，`cd frontend && pnpm vitest run src/components/tasks/follow-up` 202 条通过，`pnpm run frontend:check`、`pnpm run frontend:lint` 通过。
- 2026-06-13 / Phase 2 T2.13：完成图片内联展示与生成图片卡收口。用户消息中的 `.vibe-images/*` markdown 图片会拆出为 inline thumbnail，点击复用 `ImagePreviewDialog`，并通过 `useImageMetadata` 读取 proxy、文件名、格式与大小；`GeneratedImagesBlock` 支持 data/http/blob 与 `.vibe-images` metadata proxy 图片，显示状态、原始 prompt、修订 prompt、错误信息、原始结果，并提供生成图预览按钮。验证：`cd frontend && pnpm exec vitest run src/components/NormalizedConversation/UserMessage.test.tsx src/components/NormalizedConversation/tools/ToolCards.test.tsx` 19 条通过，`pnpm run frontend:check`、`pnpm run frontend:lint` 通过。
- 2026-06-13 / Phase 2 T2.12：完成 TurnStats 与 LiveTurnStats。新增 `conversation-thread/TurnStats.tsx`、`LiveTurnStats.tsx` 与 `turnStatsModel.ts`，在 assistant turn 后显示复制、模型、token/context window、缓存读写 token、耗时、完成时间与回跳上一条用户消息；缺失字段自动隐藏。`VirtualizedList` 复用虚拟 row 索引执行回跳，并为运行中的 Agent turn 叠加 live usage 与实时耗时；`conversationTokenUsage.ts` 增加 Agent usage 事件到 token snapshot 的统一适配。验证：`cd frontend && pnpm exec vitest run src/components/conversation-thread/TurnStats.test.tsx src/hooks/useConversationHistory/conversationTokenUsage.test.ts src/components/logs/VirtualizedList.test.ts src/features/agents/store.test.ts` 33 条通过，`pnpm run frontend:check`、`pnpm run frontend:lint` 通过。
- 2026-06-13 / Phase 2 T2.11：完成消息导航轨。新增 `conversation-thread/messageNavEntries.ts` 生成每条用户消息的虚拟 row 锚点、预览文案与后续文件变更 +N/-N 统计；新增 `ConversationMessageNav.tsx` 宽屏 sticky 导航轨；`VirtualizedList` 维护当前虚拟锚点，点击导航项通过 `rowVirtualizer.scrollToIndex` 跳转并高亮当前用户消息 row。验证：`cd frontend && pnpm exec vitest run src/components/conversation-thread/messageNavEntries.test.ts src/components/conversation-thread/ConversationMessageNav.test.tsx src/components/logs/VirtualizedList.test.ts` 18 条通过，`pnpm run frontend:check`、`pnpm run frontend:lint` 通过。
- 2026-06-13 / Phase 2 T2.10：完成 Thinking 卡升级。`ThinkingEntry` 支持流式默认展开、耗时显示、结束后默认收起，并通过 existing expandable store 保留用户手动展开/收起偏好；`AggregatedThinkingCard` 增加聚合耗时与 count 展示；`ContentPartsRenderer` 传递 `isStreaming`/`elapsedMs`。验证：`cd frontend && pnpm exec vitest run src/components/NormalizedConversation/ThinkingEntry.test.tsx src/lib/conversation-rendering/adaptContentParts.test.ts` 17 条通过，`pnpm run frontend:check`、`pnpm run frontend:lint` 通过。
- 2026-06-13 / Phase 2 T2.9：完成第二批工具卡分型。新增 `PlanCard`、`UnifiedDiffPreview`、`AskQuestionResultCard`、`FeedbackCheckResultCard`、`GoalToolCall`、`GeneratedImagesBlock`，并在 `ToolCallCard`/`DisplayConversationEntry` 中按 action/tool name 分发；plan 支持状态、优先级和运行中样式，file_edit 进入内联 unified diff，question/feedback/goal/generated image 均保留参数与原始结果可检查。验证：`cd frontend && pnpm exec vitest run src/components/NormalizedConversation/tools/ToolCards.test.tsx` 15 条通过，`pnpm run frontend:check`、`pnpm run frontend:lint` 通过。
- 2026-06-13 / Phase 2 T2.3：完成 Markdown 管线的 Streamdown 等价 fallback 层。新增 `lib/conversation-rendering/streamdownPlugins.ts` 集中处理 tag reference、裸图片路径、TeX delimiter、未闭合 fenced code 稳定化与 user soft breaks；`Markdown.tsx` 改为消费统一前处理结果，`ContentPartsRenderer` 传递 `softBreaks` 语义。未闭合 `mermaid` fence 在流式阶段降级为 plain code，避免每个 chunk 触发 Mermaid render。验证：`cd frontend && pnpm exec vitest run src/lib/conversation-rendering/streamdownPlugins.test.ts src/components/NormalizedConversation/Markdown.test.tsx` 26 条通过，`pnpm run frontend:check` 通过。
- 2026-06-13 / Phase 2 T2.2：完成依赖与长会话渲染 fixture 收口。根据 T2.1 裁剪决策，仅新增后续 Composer 命令面板需要的 `cmdk`，未安装 Streamdown/virtua/use-stick-to-bottom 等未被生产代码使用的 spike 依赖；`longConversation` fixture 扩展为 CJK、代码块、数学、Mermaid、命令/文件/搜索/web/通用 JSON 工具卡、inline diff、Thinking、用户图片与 1,000 rows 长列表压力样本。验证：`cd frontend && pnpm install --lockfile-only`、`pnpm exec vitest run src/components/NormalizedConversation/__fixtures__/longConversation.test.ts`、`pnpm run frontend:check` 通过。
- 2026-06-13 / Phase 2 T2.1：完成 React 18 渲染栈兼容性 spike。临时 Vite + React 18 demo 验证 `streamdown`、`@streamdown/*`、`shiki`、`use-stick-to-bottom`、`virtua` 可完成类型检查与生产构建；同时记录静态接入 Streamdown 全插件栈会显著膨胀首屏 chunk，因此 T2.2/T2.3 采用 VibeX 已落地的 `react-markdown` + Shiki + KaTeX + Mermaid 等价 fallback 层，暂不安装未被生产代码使用的 spike 依赖。决策见 `02-conversation-rendering/spike-result.md`。
...
|   12 | Token/成本统计           | `message/turn-stats.tsx`、`live-turn-stats.tsx`         | 已通过 `TurnStats`/`LiveTurnStats` 接入 assistant turn 后元信息行           | Phase 1 + Phase 2 / T2.12      | Usage 事件贯通，显示模型、token、耗时、复制、回跳                         |
...
|   15 | 自定义滚动条             | `overlayscrollbars`                                     | Phase 2 已裁剪，保留原生滚动条                                             | Phase 2 / T2.15                | 不引入 scroll wrapper 破坏虚拟滚动；后续具备浏览器回归后再评估            |
|   16 | 命令面板                 | `cmdk`                                                  | 无                                                                        | Phase 2 / T2.14                | `/`、`@`、全局命令统一键盘语义                                            |
...
|   19 | 生成图片卡片             | `generated-images-block.tsx`                            | 已支持状态、修订 prompt、错误、缩略图与预览                                | Phase 2 / T2.13                | 生成中/成功/失败、修订 prompt、缩略图与打开预览                           |
|   20 | 图片内联展示             | `user-image-attachments.tsx`                            | 已从 `.vibe-images` markdown 拆出 inline thumbnail                         | Phase 2 / T2.13                | 用户附件在消息内缩略展示，点击仍复用现有预览弹窗                          |

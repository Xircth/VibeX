...
Vibe Ultra creates a **git worktree** - a separate working directory with its own branch. This keeps your changes isolated from your main codebase. Your original repository remains untouched.
</Step>
...
<Info>
**Where do workspaces live?** Worktrees are created in a `.vibe-ultra-workspaces` directory (configurable in Settings → General → Workspace Directory). Each workspace gets its own folder.
</Info>
...
3. Check that Vibe Ultra has permission to access the folder
</Accordion>

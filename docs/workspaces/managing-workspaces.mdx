...
|----------|-----------------|
| **macOS** | `/var/folders/.../T/vibe-ultra/worktrees` (system temp) |
| **Linux** | `/var/tmp/vibe-ultra/worktrees` |
| **Windows** | `%TEMP%\vibe-ultra\worktrees` |

You can configure a custom location in **Settings → General → Workspace Directory**. When set, worktrees are stored in `{your-path}/.vibe-ultra-workspaces`.
...
<Tab title="macOS">
```bash
du -sh $TMPDIR/vibe-ultra/worktrees
```
</Tab>
<Tab title="Linux">
```bash
du -sh /var/tmp/vibe-ultra/worktrees
```
</Tab>
<Tab title="Windows">
```powershell
Get-ChildItem "$env:TEMP\vibe-ultra\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
...
If you manually delete a worktree directory (e.g., using `rm -rf` in terminal), Vibe Ultra will **automatically recreate it** the next time you open that workspace. The worktree is recreated from the branch's last commit.
...
Vibe Ultra automatically cleans up "orphaned" worktrees on startup - these are worktree directories that no longer have a matching workspace in the database (e.g., if the app crashed during deletion).

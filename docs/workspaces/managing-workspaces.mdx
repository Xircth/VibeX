...
|----------|-----------------|
| **macOS** | `/var/folders/.../T/vibex/worktrees` (system temp) |
| **Linux** | `/var/tmp/vibex/worktrees` |
| **Windows** | `%TEMP%\vibex\worktrees` |

You can configure a custom location in **Settings 鈫?General 鈫?Workspace Directory**. When set, worktrees are stored in `{your-path}/.vibex-workspaces`.
...
<Tab title="macOS">
```bash
du -sh $TMPDIR/vibex/worktrees
```
</Tab>
<Tab title="Linux">
```bash
du -sh /var/tmp/vibex/worktrees
```
</Tab>
<Tab title="Windows">
```powershell
Get-ChildItem "$env:TEMP\vibex\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
...
If you manually delete a worktree directory (e.g., using `rm -rf` in terminal), VibeX will **automatically recreate it** the next time you open that workspace. The worktree is recreated from the branch's last commit.
...
VibeX automatically cleans up "orphaned" worktrees on startup - these are worktree directories that no longer have a matching workspace in the database (e.g., if the app crashed during deletion).

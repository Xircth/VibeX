...
The available slash commands depend on which coding agent you're using. VibeX discovers native commands, then exposes the subset that can produce a useful chat-visible result.

<Note>
  Some built-in commands that require TUI interaction (such as `/model` or
  `/theme`) are not currently supported in VibeX. Only non-interactive commands
  are available.
</Note>
...
| `/goal`    | Manage a long-running goal                              |

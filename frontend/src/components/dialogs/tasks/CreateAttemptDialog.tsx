...
const CreateAttemptDialogImpl = NiceModal.create<CreateAttemptDialogProps>(
  ({ taskId }) => {
    ...
    const defaultProfile: ExecutorProfileId | null = useMemo(() => {
      ...
      return config?.executor_profile ?? getFirstAvailableProfile(profiles);
    }, [latestAttempt?.session?.executor, config?.executor_profile, profiles]);
    ...
    return (
      <Dialog open={modal.visible} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{'创建尝试'}</DialogTitle>
            <DialogDescription>
              {'使用编码代理开始新的尝试。将创建 git 工作树和任务分支。'}
            </DialogDescription>
          </DialogHeader>

...
const CreatePRDialogImpl = NiceModal.create<CreatePRDialogProps>(
  ({ attempt, task, repoId, targetBranch }) => {
    ...
    const prDescriptionAgentConfigured = Boolean(
      config?.pr_auto_description_agent_id?.trim()
    );
    const [autoGenerateDescription, setAutoGenerateDescription] = useState(
      (config?.pr_auto_description_enabled ?? false) &&
        prDescriptionAgentConfigured
    );
    ...
    useEffect(() => {
      ...
      setPrTitle(`${task.title} (${APP_NAME})`);
    ...
    const handleConfirmCreatePR = useCallback(async () => {
      ...
      if (result.success) {
        ...
        setAutoGenerateDescription(
          (config?.pr_auto_description_enabled ?? false) &&
            Boolean(config?.pr_auto_description_agent_id?.trim())
        );
        setCreatingPR(false);
      ...
      if (result.error) {
        if (
          ...
          const gitCliErrorMessage =
            result.error.type === 'git_cli_not_logged_in'
              ? t('createPr.gitNotAuthenticated')
              : t('createPr.gitCliNotInstalled');

          setError(result.message || gitCliErrorMessage);
          setGhCliHelp(null);
          return;
        } else if (result.error.type === 'target_branch_not_found') {
          setError(
            t('createPr.targetBranchNotFound', {
      ...
      if (result.message) {
        ...
        setError(t('createPr.createFailed'));
        setGhCliHelp(null);
      }
    }, [
      attempt,
      ...
      config?.pr_auto_description_agent_id,
      modal,
      isMacEnvironment,
      t,
    ]);
            ...
            {!isLoaded ? (
              ...
              <div className="space-y-4 py-4">
                <div className="flex items-center space-x-2">
                  ...
                  >
                    {t('createPr.autoGenerate')}
                  </Label>
                </div>
                <div className="space-y-2">
                  <Label>{t('createPr.titleLabel')}</Label>
                  <TextInput
                    ...
                    isDisabled={autoGenerateDescription}
                    width="100%"
                ...
                <div className="space-y-2">
                  ...
                  <BranchSelector
                    ...
                    placeholder={
                      branchesLoading
                        ? t('createPr.loadingBranches')
                        : t('createPr.selectBaseBranch')
                    }
                    className={

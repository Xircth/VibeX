  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          DRAG_HANDLE_CLASS,
          'flex shrink-0 cursor-grab items-center gap-2 px-3 active:cursor-grabbing',
          collapsed ? 'h-full' : 'h-11'
        )}
        style={collapsed ? { height: GROUP_COLLAPSED_HEIGHT } : undefined}
      >
        <Folder className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
        {editing ? (
          <input
            value={draft}
            className="nodrag nopan min-w-0 flex-1 rounded-md border border-border bg-[var(--surface-control)] px-1.5 py-0.5 text-xs font-semibold"
            autoFocus
          ...
          <button
            type="button"
            className="nodrag min-w-0 flex-1 truncate text-left text-xs font-semibold"
            onDoubleClick={(event) => {

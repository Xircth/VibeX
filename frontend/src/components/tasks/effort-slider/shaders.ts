/**
 * GLSL sources for the effort-slider fire effect, ported verbatim from the
 * claude-range-slider reference implementation (a replica of Claude Code's
 * effort slider): full-screen triangle vertex shader plus three fragment
 * passes — cell-grid fire simulation, reusable 7-tap gaussian blur, and the
 * final tone-mapped composite.
 */
...
export const FRAG_SIM = `#version 300 es
  ...
  void main(){
    ...
    vec3 ember = vec3(0.11, 0.20, 0.37);
    vec3 wblu  = vec3(0.25, 0.42, 0.77);
    vec3 wht   = vec3(0.94, 0.98, 1.0);
    float temp = 1.0 - dn;
    vec3 col   = mix(ember, wblu, temp);
    ...
    col += wblu * exp(-pow((uv.x - u_slider) * 3.5, 2.0)) * 0.12 * act * es;
    col *= cell;
    col *= fade_mask;

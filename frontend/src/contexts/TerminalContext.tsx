...
interface TerminalContextType {
  ...
  // Terminal connection management (Tauri PTY)
...
export function TerminalProvider({ children }: TerminalProviderProps) {
  ...
  // Store Tauri PTY connections in a ref to persist across component remounts
  ...
  const closeTerminalConnection = useCallback(async (tabId: string) => {
    const conn = terminalConnectionsRef.current.get(tabId);
    if (conn) {
      // Unsubscribe from Tauri event listener
      conn.unlisten();
  ...
  const closeTab = useCallback(
    (workspaceId: string, tabId: string) => {
      ...
      // Close the Tauri PTY connection
  ...
  const createTerminalConnection = useCallback(
    async (
      ...
      // Get initial terminal size from the terminal instance (if already registered)
      const termInstance = terminalInstancesRef.current.get(tabId);
      const cols = termInstance?.terminal.cols ?? 80;

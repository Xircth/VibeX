...
describe('buildTurnsFromEvents', () => {
  it('reconstructs user + assistant turns from the event stream', () => {
    const { turns } = buildTurnsFromEvents(
      [
        promptStarted(1, 'p1', 'Inspect repo'),
        event(2, { kind: 'message_chunk', content: { kind: 'text', text: 'Looking ' } }),
        event(3, { kind: 'message_chunk', content: { kind: 'text', text: 'now' } }),
        event(4, { kind: 'prompt_finished', finished: { prompt_id: 'p1', stop_reason: 'end_turn' } }),
      ],
      'c'
    );
    ...
    // p1 finished -> assistant turn is completed.
    expect(turns[1].completed_at).not.toBeNull();
  });

  it('keeps the in-flight (unfinished) assistant turn open with in-progress tools', () => {
    const { turns, inProgressToolCallIds } = buildTurnsFromEvents(
      [
        promptStarted(1, 'p1'),
        event(2, { kind: 'tool_call', tool_call: { id: 't1', title: 'read', kind: 'read' } }),
        event(3, {
    ...
    expect(turns).toHaveLength(2); // user + (open) assistant
  ...
  it('reconstructs multiple rounds in order', () => {
    const { turns } = buildTurnsFromEvents(
      [
        ...
        event(2, { kind: 'message_chunk', content: { kind: 'text', text: 'first' } }),
        event(3, { kind: 'prompt_finished', finished: { prompt_id: 'p1' } }),
        promptStarted(4, 'p2', 'two'),
        event(5, { kind: 'message_chunk', content: { kind: 'text', text: 'second' } }),
      ],
      'c'
    );
    expect(turns.map((t) => t.role)).toEqual(['user', 'assistant', 'user', 'assistant']);
    expect(turns[3].completed_at).toBeNull(); // p2 still in-flight
  });
});
...
describe('getTimelineTurns', () => {
  it('renders persisted turns when there are no live events', () => {
    ...
    expect(timeline.map((e) => [e.turn.id, e.phase])).toEqual([
      ['u1', 'persisted'],
      ['a1', 'persisted'],
    ]);
  });
  ...
  it('uses live turns and marks the in-flight assistant as streaming', () => {
    ...
    expect(timeline.map((e) => e.phase)).toEqual(['persisted', 'streaming']);
  ...
  it('prepends persisted history that precedes the live conversation', () => {
    ...
    const liveUser = { ...turn('u-c-p1', 'user'), timestamp: '2026-06-11T00:00:01.000Z' };
    const timeline = getTimelineTurns({
      conversationId: 'c',
      persisted: [old],
      live: [liveUser],
    });
    expect(timeline.map((e) => e.turn.id)).toEqual(['old', 'u-c-p1']);
  });
});

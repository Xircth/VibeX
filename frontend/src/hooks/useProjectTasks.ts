...
/**
 * Stream tasks for a project via Tauri events (JSON Patch) and expose as array + map.
...
export const useProjectTasks = (projectId: string): UseProjectTasksResult => {
  ...
  const { data, isConnected, isInitialized, error } = useTauriPatchStream<TasksState>({

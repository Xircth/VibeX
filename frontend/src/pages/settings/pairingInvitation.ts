...
export function pairingVisibleOrigins(
  ...
  const lan = origins.filter((item) => item.kind === 'lan');
  return lan.length > 0 ? lan : origins.slice(0, 1);
}

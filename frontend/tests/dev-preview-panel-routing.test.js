...
test('panel 注册表区分文件预览与开发预览组件', () => {
  ...
  assert.match(source, /import\('\@\/components\/panels\/DockviewPreviewPanel'\)/);
  assert.match(source, /import\('\@\/components\/panels\/DockviewDevPreviewPanel'\)/);
  assert.match(source, /\[PANEL_IDS\.PREVIEW\]:\s*LazyPreviewPanel/);
...
test('Dockview 开发预览 panel 包装组件补齐预览所需上下文', () => {
  ...
  assert.match(source, /ClickedElementsProvider/);

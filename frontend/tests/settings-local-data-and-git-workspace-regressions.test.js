...
test('system settings exposes local data clear action and calls the config api', () => {
  ...
  assert.match(source, /useWindowProjectsStore\.getState\(\)\.resetProjectWindowState\(\)/);
  assert.match(source, /toast\.warning/);
  assert.match(source, /toast\.loading/);
  assert.match(source, /toast\.dismiss/);
  assert.match(source, /!window\.location\.pathname\.startsWith\('\/settings'\)/);
  assert.match(source, /window\.location\.reload\(\)/);
  assert.match(apiSource, /clearLocalData: async \(\): Promise<ClearLocalDataResponse>/);
...
test('repo registration and worktree cleanup protect user project directories', () => {
  ...
  assert.match(containerSource, /WorkspaceManager::is_app_owned_workspace_dir\(workspace_root\)/);
  assert.match(containerSource, /External worktree path does not exist/);
  const workspaceCommandSource = readRepoFile('src-tauri/src/commands/workspaces.rs');
  ...
  assert.match(workspaceCommandSource, /imported_single_repo_agent_working_dir/);
  const sessionCommandSource = readRepoFile('src-tauri/src/commands/sessions.rs');
  assert.match(sessionCommandSource, /workspace_container_overlaps_repo/);
  assert.match(sessionCommandSource, /find_matching_project_worktree_workspace/);
...
test('git hooks and panels key their data by workspace id so git state follows the active workspace', () => {
  ...
  const changeTargetSource = readFrontendFile('src/hooks/useChangeTargetBranch.ts');

  assert.match(statusSource, /\['gitStatus', workspaceId, repoId\]/);
  assert.match(statusSource, /workspaceId\s*\?\s*await attemptsApi\.getGitStatus/);
  assert.match(diffsSource, /\['gitDiffs', workspaceId, repoId\]/);
  assert.match(diffsSource, /workspaceId\s*\?\s*await attemptsApi\.getFileDiffs/);

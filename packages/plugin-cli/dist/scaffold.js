...
async function writeSkill(root) {
    ...
    await writeFile(join(root, "contents", "skills", "hello", "SKILL.md"), `---\nname: hello\ndescription: Greet the user from this plugin.\n---\n# Hello\n\nUse this skill to greet the user.\n`);
}
async function writeWorkflow(root) {
...
async function writeManagedMcp(root) {
    ...
    const server = "process.stdout.write(JSON.stringify({ ready: true }) + '\\n');\n";
    await writeFile(join(root, "runtime", "mcp.mjs"), server);
    await writeFile(join(root, "dist", "mcp", "hello.mjs"), server);
}
async function writeHook(root) {
    await mkdir(join(root, "contents", "hooks"), { recursive: true });
    await writeFile(join(root, "contents", "hooks", "before-prompt.md"), "Follow this plugin's standing instructions before answering.\n");
}
async function writeNodeWorker(root, template, spec) {
    await mkdir(join(root, "runtime"), { recursive: true });
...
`;
    if (template === "ts-worker") {
        ...
        await writeFile(join(root, "runtime", "worker.mjs"), 'export { default } from "./worker.ts";\n');
...
function nodeWorkerSource(spec) {
...
async function writePythonWorker(root, id) {
    await mkdir(join(root, "runtime"), { recursive: true });
    await writeFile(join(root, "runtime", "worker.py"), `from vibex_plugin import define_plugin_worker, run_stdio_plugin_worker
...
async function writeRustWorker(root, id) {
    await mkdir(join(root, "runtime", "src"), { recursive: true });
    await writeFile(join(root, "runtime", "Cargo.toml"), `[package]
...
vibex-plugin-sdk = "1.0.0"
`);
    await writeFile(join(root, "runtime", "src", "main.rs"), `use serde_json::json;
...
async function writePluginTest(root, handlers) {
    await mkdir(join(root, "test"), { recursive: true });
    if (!handlers) {
        await writeFile(join(root, "test", "fixture.mjs"), `export const MANIFEST = ${JSON.stringify(JSON.parse(await (await import("node:fs/promises")).readFile(join(root, ".vibex-plugin", "plugin.json"), "utf8")), null, 2)};
`);

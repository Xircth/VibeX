...
async function validateEntrypoints(root, value, diagnostics) {
    ...
    if (isObject(value.worker)) {
        if (value.worker.format !== "javascript-esm" ||
            ...
            try {
                ...
                if (isObject(resource) && "managedRuntime" in resource) {
                    ...
                    if (!isObject(managed) ||
                        ...
                        diagnostics.push(error("managed_mcp_invalid", "Managed MCP requires a safe entrypoint, protocolRevision 2026-07-28, and an optional all-compatible-agents default binding"));
                    }
                }
            }
            catch {

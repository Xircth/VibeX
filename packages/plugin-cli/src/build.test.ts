...
describe('clean-room App + Agent build', () => {
  it('builds its own scaffold into a self-contained surface document', async () => {
    ...
    expect(manifest.integrations.some(({ kind }) => kind === 'app.surface')).toBe(
      true
    );
  ...
  it.each(['app', 'agent'] as const)(
    ...
    async (template) => {
      ...
      expect(validation.valid).toBe(true);
      expect(Boolean(manifest.entrypoints.app)).toBe(template === 'app');
      expect(manifest.integrations.some(({ kind }) => kind === 'app.surface')).toBe(
        template === 'app'
      );
      expect(
        manifest.integrations.some(({ kind }) => kind === 'workflow.binding')
      ).toBe(
        template === 'agent'
      );
    }

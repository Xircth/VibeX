...
const LIVE_KIND_ALIASES: Record<string, readonly string[]> = {
...
function liveHasKind(live: Set<string>, kind: string): boolean {
  if (live.has(kind) || live.has(catalogKindFor(kind))) return true;

...
This package lives in the VibeX monorepo as the Host-family Workflow MCP
(`vibex-workflow-mcp`). It was migrated from the standalone
`~/Projects/vibe-workflow-creator` project. The Host sibling binary in
`crates/vibex-workflow-mcp` launches the bundled `dist/mcp/workflow-control.mjs`
with Node. The checked-in `vendor/` archives pin the VibeX Plugin SDK and CLI
...
pnpm test:mcp
pnpm build
pnpm exec vibex-plugin validate .
pnpm exec vibex-plugin pack . --output vibex-workflow-creator-1.0.0.vxp
```

For a live Host verification, enable this plugin in VibeX first so VibeX adds
...
`VIBEX_WORKSPACE_ROOT` when the MCP should resolve project-relative Workflow

//! Single bridge between the legacy executor key (`AgentKind`) and the
//! ACP-native agent registry (`AgentKind`).
//!
...
//! `crates/api-types/src/agent_kind.rs` (`AgentKind::from_lenient`).

use executors::executors::AgentKind;

use crate::error::AppError;

/// Resolve a legacy executor to its ACP-native [`AgentKind`], or a `BadRequest` when
/// the executor has no ACP registry entry (i.e. it is not available through the
/// agent runtime).
pub(crate) fn agent_type_from_executor(executor: AgentKind) -> Result<AgentKind, AppError> {
    AgentKind::from_lenient(&executor.to_string()).ok_or_else(|| {
